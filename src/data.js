const data = [
  {
    id: 1,
    img: "../images/cafe_search.png",
    app_title: "Cafe Search App",
    category: "Python",
    tech_used: ["Python,", " Full Stack,", " Flask,", " API,", " SQLAlchemy"],
    description:
      "Web app for searching cafes nearby with data on power socket availability, wifi speed and coffee quality. Possible to add a new cafe through /add route",
    url: "https://github.com/dana0623/coffee-wifi",
    github_url: "https://github.com/dana0623/coffee-wifi",
  },
  {
    id: 2,
    img: "../images/youtube-clone.png",
    category: "React",
    app_title: "YouTube Clone",
    tech_used: ["React,", " Material UI,", " Front End,", " React Router"],
    description: "Front End development: Youtube Main Page and Search Page",
    url: "https://resilient-manatee-9c3893.netlify.app",
    github_url: "https://github.com/dana0623/youtube-clone",
  },
  {
    id: 3,
    img: "../images/mapty2.png",
    category: "JavaScript",
    app_title: "Mapty",
    tech_used: [
      "JavaScript,",
      " Geolocation API,",
      " External Libraries,",
      " OOP",
    ],
    description:
      "Web app to log and show activities on the map, using geolocation",
    url: "https://github.com/dana0623/geolocation-app",
    github_url: "https://github.com/dana0623/geolocation-app",
  },
  {
    id: 4,
    img: "../images/amazon-clone.png",
    category: "React",
    app_title: "Amazon Clone",
    tech_used: [
      "React,",
      " Redux,",
      " User Authorization,",
      " Firebase,",
      " Payment Checkout,",
      " Full Stack,",
      " React Router",
    ],
    description:
      "Full stack e-commerce shop development with user authentication, login and sign out, shopping cart and payment checkout",
    url: "https://clone-8ad6c.web.app/",
    github_url: "https://github.com/dana0623/amazon-clone",
  },
  {
    id: 5,
    img: "../images/forkify-img.png",
    category: "JavaScript",
    app_title: "Forkify",
    tech_used: ["JavaScript,", " API,", " Modules,", " OOP,", " Parcel"],
    description:
      "Web app for registering and searching recipes. Made using asynchronous JavaScript. ",
    url: "https://github.com/dana0623/recipe-app",
    github_url: "https://github.com/dana0623/recipe-app",
  },
  {
    id: 6,
    img: "../images/bankist.png",
    category: "JavaScript",
    app_title: "Bankist",
    tech_used: ["JavaScript,", " HTML, ", "CSS"],
    description:
      "Minimalistic online banking application that allows user to login, transfer money, request a loan and delete account.",
    url: "https://github.com/dana0623/onlinebanking-app",
    github_url: "https://github.com/dana0623/onlinebanking-app",
  },
  {
    id: 7,
    img: "../images/travel-blog.png",
    category: "Python",
    app_title: "Travel Blog Template",
    tech_used: [
      "Python, ",
      "HTML, ",
      "CSS, ",
      "Full Stack, ",
      "User Authentication",
      ", SQLite",
    ],
    description:
      "Travel blog template with a function of user sign up, login, leaving comments, add blog posts and delete them.",
    url: "https://github.com/dana0623/blog-template",
    github_url: "https://github.com/dana0623/blog-template",
  },
];

export default data;
